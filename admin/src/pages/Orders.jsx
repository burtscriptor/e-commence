...
const Orders = ({ token }) => {
  ...
  const statusHandler = async (event, orderId) => {
    ...
    try {
      ...
      if (response.data.success) {
        // No need to refresh the orders after status update, as we have already optimistically updated the state
        toast.success('Order status updated');
      } else {
        toast.error('Failed to update status');
      }
    } catch (error) {
      // If the request fails, roll back the status update
      const rollbackOrders = orders.map(order =>
        order._id === orderId ? { ...order, status: event.target.defaultValue } : order
      );
